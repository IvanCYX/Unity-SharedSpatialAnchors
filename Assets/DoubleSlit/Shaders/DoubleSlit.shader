Shader "Unlit/DoubleSlit" {
    Properties {
        //Quality properties 
        [Header(Quality)]
        _StepSize ("Step Size", Range(0.001,0.01)) = 0.001
        _Scale ("Scale", Range(0.5,1)) = 0.5
        _Height ("Height", Range(0,1)) = 0.537
        _Width ("Width", Range(0,1)) = 0.577
        _Outline ("Outline Strength", Range(0,1)) = 0.102

        //Transfer function properties
        [Header(Transfer Function)]
        _ProbabiltyDensityCutoffR ("Probabilty Density Cutoff R", Range(0,1)) = 0.19
        _ProbabiltyDensityCutoffG ("Probabilty Density Cutoff G", Range(0,1)) = 0.069
        _ProbabiltyDensityCutoffB ("Probabilty Density Cutoff B", Range(0,1)) = 0.021

        //Plane wave properties
        [Header(Plane Wave)]
        _waveNumber ("Wave number k", Range(10, 50)) = 46.6
        _angularVelocity ("Angular Velocity w", Range(0, 5)) = 5
        _gaussianCoefficient ("Gaussian Coefficient", Range(1, 50)) = 50

        //Interfernce wave properties
        [Header(Interference Wave)]
        _slitSpacing ("Slit Spacing", Range(0, 1)) = 0.439
        _widthScale ("Width Scale", Range(1, 5)) = 2.85
        _intensityScale ("Intensity Scale", Range(0, 1)) = 0.34
    }
    ...
    SubShader {
        ...
        Pass {
            ...
            bool checkInnerBoundingBox(float3 position) {
                return abs(position.x) < 0.5 + Epsilon && abs(position.y) < 0.5 * _Height + Epsilon && abs(position.z) < 0.5 * _Width + Epsilon;
            }

            //Wave functions
            float PlaneWave(float3 position) {
                return sin(position.x * _waveNumber - _Time.y * _angularVelocity);
            }

            float InterferenceWave(float3 position) {
                position.z *= _widthScale;
                return cos(length(position.xz - float2(0, _slitSpacing/2)) * _waveNumber - _Time.y * _angularVelocity) + cos(length(position.xz + float2(0, _slitSpacing/2)) * _waveNumber - _Time.y * _angularVelocity);
            }
            ...
            float GaussianPlaneWave(float3 position) {
                ...
                return PlaneWave(position) * exp(-_gaussianCoefficient*(position.y * position.y));
            }
            ...
            float GaussianInterferenceWave(float3 position) {
                ...
                return InterferenceWave(position) * exp(-_gaussianCoefficient*(position.y * position.y)) * _intensityScale;
            }
